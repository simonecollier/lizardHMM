...
#' @param @param zweight0 The starting values for the weights of the zero
#'   inflated portion of the gamma distributions. `zweight0` is a list of
#'   vectors, one for each variable, and each vector element correspomnds to a
#'   different subject.
...
gam0_fit_hmm <- function(x, design, num_states, num_variables, num_subjects,
                         ...
                         steptol = 1e-6, hessian = FALSE, print.level = 0) {
  num_time <- nrow(x)
  working_params <- gam0_working_params(num_states, num_variables, num_subjects,
                                        alpha0, theta0, zweight0, beta0, delta0)
  hmm   <- stats::nlm(gam0_loglikelihood,
                      working_params,
                      x = x,
                      design = design,
